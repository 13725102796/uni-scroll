<template>
	<view :style="{'padding-top':padTop,'padding-bottom':padBottom}" @touchstart="touchstartEvent" @touchmove="touchmoveEvent" @touchend="touchendEvent" @touchcancel="touchendEvent">
		...
		<view v-if="optDown" class="mescroll-downwarp" :class="{'mescroll-downwarp-reset':isDownReset}" :style="{'height': downHight+'px', 'position': 'relative', 'overflow': 'hidden', '-webkit-transition': isDownReset?'height 300ms':''}">
			<view class="downwarp-content" style="text-align: center;position: absolute;left: 0;bottom: 0;width: 100%;padding: 20upx 0;">
				...
				<view >
					<image :src="progressImg" mode="aspectFit" class="downwarp-progress" :class="{'mescroll-rotate':isDownLoading}"  :style="{'transform':'rotate(' + downRotate + 'deg)'}"></image>
					<image :src="sloganImg" mode="aspectFit" class="downwarp-slogan" ></image>
				</view>	
			</view>
		</view>
		...
		<view v-if="optUp" class="mescroll-upwarp">
			...
			<template v-if="isUpLoading">
				<image :src="progressImg" mode="aspectFit" class="downwarp-progress mescroll-rotate"  :style="{'transform':'rotate(' + downRotate + 'deg)'}"></image>
				<image :src="sloganImg" mode="aspectFit" class="downwarp-slogan" ></image>
	<!-- 			<view class="upwarp-progress mescroll-rotate"></view>
			...
			<view v-if="isUpNoMore" class="upwarp-nodata">
				<image :src="nodataImg" mode="aspectFit" class="downwarp-slogan" ></image>
			<!-- {{optUp.textNoMore}} -->
			</view>
			<view v-if="!isUpNoMore" style="height: 100upx"></view>
		</view>
...
<script>
	...
	import progressImg from './img/mescroll-progress.png'
	import sloganImg from './img/mescroll-slogan.png'
	import nodataImg from './img/mescroll-nodata.png'
	export default {
		data() {
			// progressImg;
			// sloganImg;
